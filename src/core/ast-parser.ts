...
export class SimpleImportAstParser implements AstParser {
  public parseImports(
    ...
    this.updateFirstNodeLeadingComments(
      importsAndTypes.importNodes,
      sourceText
    );
  ...
  private updateFirstNodeLeadingComments(
    ...
    if (!firstNode) return;
    else if (!firstNode.importComment.leadingComments.length) return;
  ...
  private firstImportLineNumber(importNode: ImportNode, text: string) {
    ...
    if (leadingComments)
      return textProcessing.getPositionByOffset(leadingComments.range.pos, text)
        .line;

    return importNode.start.line;
  }

  private getLastLeadingComment(importNode: ImportNode): Comment | null {
    if (!importNode) return null;
  ...
  private delintImportsAndTypes(
    ...
    const delintNode = (node: ts.Node) => {
      ...
      if (!isSkipChildNode) ts.forEachChild(node, delintNode);
    };
  ...
  private getCodeLineNumbers(node: ts.Node, sourceFile: ts.SourceFile) {
    const importStartLine = sourceFile.getLineAndCharacterOfPosition(
      node.getStart(sourceFile)
    );
    const importEndLine = sourceFile.getLineAndCharacterOfPosition(
      node.getEnd()
    );
  ...
  private parseImport(
    ...
    const moduleSpecifierName =
      importNode.importDeclaration.moduleSpecifier.kind ===
      ts.SyntaxKind.StringLiteral
        ? (importNode.importDeclaration.moduleSpecifier as ts.StringLiteral)
            .text
    ...
    if (!importClause.namedBindings) return result;
    ...
    if (importClause.namedBindings.kind === ts.SyntaxKind.NamedImports) {
      ...
      nImport.elements.forEach((y) => {
        ...
        result.namedBindings?.push({ aliasName, name: propertyName });
      });
