...
export class SimpleImportRunner implements ImportRunner {
  ...
  private getSortedData(
    ...
    const sortedImports = this.sorter.sortImportElements(
      imports.importElements
    );

    const sortedImportsWithExcludedImports = this.getExcludeUnusedImports(
      sortedImports,
    ...
    const fileSourceWithSkippedLineShift =
      fileSourceWithSkippedLineShiftArray.join('\n');
    ...
    const rangesToDelete = this.getRangesToDelete(
      sortedImportsWithExcludedImports,
      fileSourceArray,
      fileSource
    );
  ...
  private getExcludeUnusedImports(
    sortResult: ImportElementSortResult,
    usedTypeReferences: string[]
  ): ImportElementExcludeUnusedResult {
    const isRemoveUnusedImports =
    ...
    const isRemoveUnusedDefaultImports =
      this.configurationProvider.getConfiguration().sortConfiguration
      ...
      gr.elements = gr.elements.filter((el) => {
        ...
        el.namedBindings = el.namedBindings?.filter((nameBinding) => {
          const isUnusedNameBinding = !usedTypeReferences.some(
            (reference) =>
              reference === (nameBinding.aliasName || nameBinding.name)
          );
          return !isUnusedNameBinding;
        });

        if (!isRemoveUnusedDefaultImports && el.defaultImportName) return true;

        if (
          isRemoveUnusedDefaultImports &&
          usedTypeReferences.some(
            (reference) => reference === el.defaultImportName
          )
        )
          return true;
  ...
  private getFullSortedSourceFile(
    sourceText: string,
    sortedData: SortedImportData
  ): string {
    const fileSourceArray = sourceText.split('\n');

    const linesToDelete = chain(
      sortedData.rangesToDelete?.map((range) =>
        rangeLodash(range.startLine, range.endLine)
      )
    )
    ...
    for (let i = linesToDelete.length - 1; i >= 0; i--)
      if (i === 0)
        fileSourceArray.splice(
          linesToDelete[i],
          1,
          sortedData.sortedImportsText!
        );
      else fileSourceArray.splice(linesToDelete[i], 1);
  ...
  private isSourceAlreadySorted(
    sortedImport: { data: string[]; text: string },
    source: { data: string[]; text: string }
  ): boolean {
    if (
      ...
      ((source.data.length > sortedImport.data.length &&
        ...
        (source.data.length === sortedImport.data.length + 1 &&
          this.isLineEmptyOrWhiteSpace(
            source.data[sortedImport.data.length]
          )) ||
        source.data.length === sortedImport.data.length) &&
  ...
  private getNextNonEmptyLine(
    ...
    fileSourceArray: string[]
  ): { lineNumber: number; isLast: boolean } | null {
    const nextLineIndex = startLineIndex + 1;
    if (fileSourceArray.length < 0) return null;
    ...
    else return this.getNextNonEmptyLine(nextLineIndex, fileSourceArray);
  }

  private getRangesToDelete(
    sortedImportsResult: ImportElementExcludeUnusedResult,
    fileSourceArray: string[],
    fileSourceText: string
  ): LineRange[] {
    const sortedImports = chain(sortedImportsResult.groups)
    ...
    chain(sortedImports)
      ...
      .forEach((x) => {
        ...
        const lastTrailingComment =
          x.importComment.trailingComments.reverse()[0];

        const startPosition = firstLeadingComment
          ? textProcessing.getPositionByOffset(
              firstLeadingComment.range.pos,
              fileSourceText
            )
          : x.startPosition;
        const endPosition = lastTrailingComment
          ? textProcessing.getPositionByOffset(
              lastTrailingComment.range.end,
              fileSourceText
            )
          : x.endPosition;
        ...
        const nextNonEmptyLine = this.getNextNonEmptyLine(
          currentRange.endLine - 1,
          fileSourceArray
        );
  ...
  private isFileExcludedFromSorting(selectedPath: string) {
    const excludedFiles =
      this.configurationProvider.getConfiguration().generalConfiguration
        .exclude || [];
    ...
    const isExcluded = excludedFiles.some(
      (fileToExclude) => filePath.match(fileToExclude) !== null
    );
